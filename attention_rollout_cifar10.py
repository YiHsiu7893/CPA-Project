...
# Loading data
train_data = CIFAR10(root='./cifar10', train=True, download=True, transform=ToTensor())
test_data = CIFAR10(root='./cifar10', train=False, download=True, transform=ToTensor())
...
img_tensor = torch.from_numpy(test_data.data[7]).to(device)
print(img_tensor.shape)
patches = patchify(img_tensor.unsqueeze(0).permute(0, 3, 1, 2), N_PATCHES).to(model.positional_embeddings.device)
...
qkv = transformer_input.reshape(65, 2, 4, 4)
...
attn_heatmap = joint_attentions[0, 1:].reshape((8, 8))
...
label=test_data.targets[7]
print(label)
ax1.set_title(label_to_class_name(label))
